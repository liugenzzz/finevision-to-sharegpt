...
**为什么**：用户一句话点破了前提错误——「人家就是并发太高返回慢，或者有高难
样本，模型一直都能通」。他是对的：**「连续失败 20 次」推不出「后端坏了」**。
高并发下响应慢、或者连着撞上一串长对话，健康的后端照样连续失败几十次，然后
被摘掉；六个全摘光，整轮任务停摆。**摘掉的往往是好的那些。**

改成按错误性质判断，不按次数判断：

- **算后端故障**：连不上、`Connection refused`、DNS/SSL、401/403、404、
  `model does not exist` —— 这些说明这个后端**用不了**，换一个才有意义。
- **不算**：超时、5xx 过载、译文解析失败 —— 这些是**这条任务没成**，
  跟后端健康与否无关，重试或换后端都不会更好。

另外加一条兜底：**最后一个还活着的后端永远不摘**。摘了等于整轮任务原地死亡，
而失败的样本本来就会进 `failed.jsonl`、之后能单独重试——**停摆比失败贵得多**。

模拟现场那种形状（6 个后端、三分之二请求超时）：5000 条全部跑完、成功 1666、
没有后端被摘、没有抛错。改之前这会在头几百条就全摘光然后抛错。

**对另一侧的影响**：`BackendResult` 多了一个 `backend_fault: bool` 字段
（默认 `False`），说明这条失败是否属于"后端本身不可用"。你要是有消费
`BackendResult` 的代码，多一个字段不影响解包。

**验证**：251 passed / 25 skipped，ruff 干净。新增 3 个测试：超时不摘后端、
难样本不算后端故障、`is_backend_fault` 的分类边界；改写 3 个旧测试——
它们用泛泛的 `RuntimeError("down")` 当"后端坏了"，编码的是旧语义。
